  ...
  return (
    <nav className= {`${darkMode ? 'dark bg-black' : 'light bg-white'} w-full flex lg:flex-col flex-row justify-between items-center gap-4 lg:px-20 px-6 lg:py-1 lg:static sticky top-0 z-[999]`}>
      <div id='top' className='relative w-full flex justify-center items-center'>
        ...
        <FaBars className='w-[25px] h-[25px] transform hover:scale-125 transition-transform duration-300 cursor-pointer' />
        <div className='relative'>
        <FiShoppingCart className='w-[25px] h-[25px] transform hover:scale-125 transition-transform duration-300 cursor-pointer' />
        <div className='bg-black dark:bg-white dark:text-black px-3 py-1 text-white rounded-full absolute -top-[25px] -right-[20px] text-sm'>2
        </div>
        </div>
        </div>
      </div>
      ...
      <div data-aos="zoom-in" id='menus section' className='w-full pb-5 flex lg:justify-center justify-end items-center sticky top-0 left-0 h-fit bg-white dark:bg-black'>
        ...
        <div className='flex justify-center items-center lg:hidden mt-5' onClick={toggleMenu}>
          <div>
            {isMenuOpen ? <FaXmark className='text-black text-3xl dark:text-white cursor-pointer' /> : <FaBars className='text-black text-3xl dark:text-white cursor-pointer'/>}
          </div>
        </div>
        ...
        <div className={`${isMenuOpen ? 'flex' : 'hidden'} w-full h-fit bg-slate-800 p-4 absolute top-[80px] left-0`}>
          ...
          {
            navItems.map(({link, path})=> (
              <Link key={path} className='text-white uppercase font-semibold cursor-pointer p-3 rounded-lg hover:bg-yellow-500 hover:text-black w-full text-center' to={path}>{link}</Link>
            ))
          }
          </ul>
        </div>
